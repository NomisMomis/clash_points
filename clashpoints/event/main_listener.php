...
namespace nomismomis\clashpoints\event;
...
class main_listener implements EventSubscriberInterface
{
	...
	public function load_language_on_setup($event)
	{
		...
		$lang_set_ext[] = array(
			'ext_name' => 'nomismomis/clashpoints',
			'lang_set' => 'common',
		);
	...
	public function add_page_header_link()
	{
		$this->template->assign_vars(array(
			'U_DEMO_PAGE'	=> $this->helper->route('nomismomis_clashpoints_controller', array('name' => 'world')),
		));
	}
	...
	public function viewonline_page($event)
	{
		if ($event['on_page'][1] === 'app' && strrpos($event['row']['session_page'], 'app.' . $this->php_ext . '/demo') === 0)
		{
			...
			$event['location_url'] = $this->helper->route('nomismomis_clashpoints_controller', array('name' => 'world'));
		}
	}
